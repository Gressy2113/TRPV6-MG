...
MPIRUN=/usr/bin/mpirun #/usr/lib64/openmpi/bin/mpirun
...
#       -gpu_id $GPU_ID -ntomp $NT1 -pin on -pinoffset $PINOFFSET -pinstride 1 \
...
nohup $MPIRUN -np $N_WALKERS gmx_mpi mdrun -plumed plmd.dat -v -deffnm md \
      -multidir walker_* \
      -nb gpu -bonded gpu -pme gpu -pmefft gpu \
      -gpu_id 0,1 -ntomp 12 -pinoffset 0 -pin on  -pinstride 1 \
      -cpi -cpo -cpt 15 > md.job & 
